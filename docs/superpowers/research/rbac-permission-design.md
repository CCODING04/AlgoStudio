...
| # | 问题 | 决策 | 依据 |
|---|------|------|------|
| 1 | Organization 默认配额 | 预设默认值 | 便于立即可用 |
| 2 | 任务删除方式 | **软删除** | 支持撤销、审计追踪 |
| 3 | 资源采集频率 | 10s 间隔 | 平衡精度与开销 |
| 4 | 审计日志保留期 | **180 天** | GDPR 合规，平衡存储成本 |

**软删除实现**: tasks 表添加 `deleted_at` 字段，查询时默认过滤已删除记录
...
| v1.1 | 2026-03-27 | 更新决策: 软删除、审计保留180天 |
